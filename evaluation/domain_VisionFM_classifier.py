...
def random_center_crop(image):
    scale = 0.95 # random.uniform(0.5, 1.0)
...
def confusion_matrix_figure(prediction,target):
    ...
    plt.savefig("../VisionFM/domain_confusion_matrix/VisionFM_high_spatial_new.png", dpi=600, bbox_inches='tight')
    plt.close()
...
def main(args):
    ...
    train_transform = self_transforms.Compose([
        ...
        SpatialHighPass(apply_prob=1),     # 30%概率应用高通
...
def validate_network(val_loader, epoch, model, linear_classifier, n, avg_pool, dst_json_path = None):
    log_file = "../VisionFM/domain_logs/Spatial_High.log"
