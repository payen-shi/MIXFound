...
def confusion_matrix_figure(prediction,target,task,seed):
    ...
    ax.set_xlabel('Predicted label', fontsize=16)  # X轴标签
    ax.set_ylabel('True label', fontsize=16)       # Y轴标签
    ax.tick_params(axis='both', which='major', labelsize=14)  # 刻度标签
    
    plt.savefig("../VisionFM/class_confusion_matrix/Retizero_Noaug_{task}_{seed}".format(task=task,seed=seed), dpi=600, bbox_inches='tight')
    plt.close()
...
def main(args):
    ...
    with open("../VisionFM/Final_feature/Retizero_feature_training_A.pickle", 'rb') as file:
        train_feats = pickle.load(file)
    with open("../VisionFM/Final_feature/Retizero_feature_test_{task}.pickle".format(task=args.Task), 'rb') as file:
        val_feats = pickle.load(file)
    train_path = '../VisionFM/dataset/fundus/training/training_labels_A.txt'
    test_path = '../VisionFM/dataset/fundus/{task}/subtest_labels.txt'.format(task=args.Task)
    ...
    print("training_set:", selected_train_feats_data.shape[0])
    ...
    print("val_set:", selected_val_feats_data.shape[0])
    ...
    if args.avgpool_patchtokens == 0:
        linear_classifier = ClsHead(embed_dim=512, num_classes=args.num_labels, layers=1)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch % args.val_freq == 0 or epoch == args.epochs - 1:
            ...
            test_stats = validate_network(val_loader,epoch, linear_classifier)
...
def validate_network(loader, epoch,linear_classifier):
    seed = args.seed
    task = args.Task
    log_file = "../VisionFM/classification_logs/Retizero_{task}_{seed}.log".format(task=task, seed=seed)
    ...
    with open(log_file, 'a', encoding='utf-8') as f:
        ...
        try:
            ...
            final_pickle_path = '../VisionFM/Final_prediction/Retizeo_pred_{task}_{seed}_new.pickle'.format(task=task, seed=seed)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--Task', default='D',choices=['A', 'B','C','D','E','F','G'], type=str)
