...
def confusion_matrix_figure(prediction,target,task,seed):
    ...
    plt.savefig("../VisionFM/class_confusion_matrix/FLAIR_Noaug_{task}_{seed}".format(task=task,seed=seed), dpi=600, bbox_inches='tight')
    plt.close()
...
def main(args):
    ...
    with open("../VisionFM/Final_feature/FLAIR_feature_A.pickle", 'rb') as file:
        train_feats = pickle.load(file)
    with open("../VisionFM/Final_feature/FLAIR_feature_{task}_new.pickle".format(task=args.Task), 'rb') as file:
...
def validate_network(loader, epoch,linear_classifier):
    seed = args.seed
    task = args.Task
    log_file = "../VisionFM/classification_logs/FLAIR_{task}_{seed}.log".format(task=task, seed=seed)
    ...
    with open(log_file, 'a', encoding='utf-8') as f:
        ...
        try:
            ...
            final_pickle_path = '../VisionFM/Final_prediction/FLAIR_pred_{task}_{seed}_new.pickle'.format(task=task, seed=seed)
