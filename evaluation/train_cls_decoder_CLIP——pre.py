...
def confusion_matrix_figure(prediction,target,task,seed):
    ...
    plt.savefig("../VisionFM/class_confusion_matrix/CLIP_{task}_{seed}".format(task=task,seed=seed), dpi=600, bbox_inches='tight')
    plt.close()
...
def validate_network(val_loader, epoch, model, linear_classifier, n, avg_pool, dst_json_path = None):
    seed = args.seed
    task = args.Task
    log_file = "../VisionFM/classification_logs/CLIP_{task}_{seed}.log".format(task=task, seed=seed)
    ...
    with open(log_file, 'a', encoding='utf-8') as f:
        ...
        try:
            ...
            final_pickle_path = '../VisionFM/Final_prediction/CLIP_pred_{task}_{seed}.pickle'.format(task=task, seed=seed)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--Task', default='A',choices=['A', 'B','C','D','E','F','G'], type=str)
