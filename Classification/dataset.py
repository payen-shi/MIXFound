...
class ClsImgs(Dataset):
    ...
    def get_entries(self, sub_datasts:list):
        ...
        for sub_dataset in sub_datasts:
            print(f"reading images at {sub_dataset}")
            if self.split in sub_dataset:
                label_file = os.path.join(sub_dataset, f'{self.split}_labels.txt')# changedlabel_file = os.path.join(sub_dataset, f'labels.txt')
    ...
    def read_txt(self, file_path, sub_dir):
        ...
        for item in data:
            item = item.strip() # remove the spac
