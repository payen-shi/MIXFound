...
def main(args):
    VisionFM_val_file_path = '../VisionFM/Final_prediction_Selection/VisionFM_pred_G_256.pickle'
    with open(VisionFM_val_file_path, 'rb') as file:
        VisionFM_val_feats = pickle.load(file)
    val_path = '../VisionFM/dataset/fundus/G/subtest_labels.txt'
    ...
    RETFound_val_file_path = '../VisionFM/Final_prediction/RETFound_pred_G_32.pickle'
    ...
    FLAIR_val_file_path ='../VisionFM/Final_prediction/FLAIR_pred_G_64.pickle'
    ...
    CLIP_val_file_path = '../VisionFM/Final_prediction_Selection/CLIP_pred_G_64.pickle'
    ...
    final_predictions = VisionFM_val_fea#fused_preds
  
    log_file = "../VisionFM/all_score_classification_logs/CLIP_Mixture_G.log"
    ...
    with open(log_file, 'a', encoding='utf-8') as f:
        ...
        try:
            ...
            preds_label = torch.argmax(preds_all, dim=1)
            print("preds_label:", preds_label)
            assert False
            print(len(txt_img_paths))
            output_dir = "../VisionFM/result_analysis"
            os.makedirs(output_dir, exist_ok=True)
            
            txt_img_paths = np.array(txt_img_paths)      # 图像路径列表
            pred_labels = preds_label.numpy()            # 预测标签数组
            true_labels = targets_all.numpy()            # 真实标签数组
            
            assert len(txt_img_paths) == len(pred_labels) == len(true_labels), \
                f"维度不匹配: 路径数({len(txt_img_paths)}), 预测数({len(pred_labels)}), 真实数({len(true_labels)})"
            
            output_path = os.path.join(output_dir, "predictions_with_labels.txt")
            
            with open(output_path, 'w', encoding='utf-8') as f:
                header = f"{'Image Path':<50} | {'Predicted Label':<14} | {'True Label':<10} | {'Match':<5}"
                f.write(header + '\n')
                f.write('-'*len(header) + '\n')
                
                for idx, (path, pred, true) in enumerate(zip(txt_img_paths, pred_labels, true_labels)):
                    is_correct = "✔" if pred == true else "✗"
                    line = f"{path:<50} | {pred:<14} | {true:<10} | {is_correct:<5}"
                    f.write(line + '\n')
            
            print(f"\n[保存完成] 预测结果已保存至：{output_path}")
            assert len(txt_img_paths) == len(preds_label)
            assert False
            print("--------scores_all--------")
