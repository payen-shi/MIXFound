...
def random_center_crop(image):
    scale = 0.95*0.4# random.uniform(0.5, 1.0)
...
def confusion_matrix_figure(prediction,target):
    ...
    disp = ConfusionMatrixDisplay(
        confusion_matrix=cm,
        display_labels=["HD", "BJ"]#"XN"]#"BJ"]#,, "YS"]  # 替换为你的实际类别名称
    )
    ...
    plt.savefig("../VisionFM/domain_confusion_matrix/VisionFM_AB_size_40.png", dpi=600, bbox_inches='tight')
    plt.close()
...
def validate_network(val_loader, epoch, model, linear_classifier, n, avg_pool, dst_json_path = None):
    log_file = "../VisionFM/domain_logs/VisionFM_AB_size_40.log"
