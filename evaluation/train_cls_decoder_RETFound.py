...
def confusion_matrix_figure(prediction,target,task,seed):
    ...
    plt.savefig("../VisionFM/class_confusion_matrix/RETFound_Noaug_{task}_{seed}".format(task=task,seed=seed), dpi=600, bbox_inches='tight')
    plt.close()
...
def main(args):
    ...
    with open("../VisionFM/Final_feature/RETFound_A.pickle", 'rb') as file:
        train_feats = pickle.load(file)
    with open("../VisionFM/Final_feature/RETFound_{task}_new.pickle".format(task=args.Task), 'rb') as file:
...
def validate_network(loader, linear_classifier,epoch):
    seed = args.seed
    task = args.Task
    log_file = "../VisionFM/classification_logs/RETFound_{task}_{seed}.log".format(task=task, seed=seed)
        ...
        try:
            ...
            for inp, target in metric_logger.log_every(loader, 20, header):
                ...
                for batch_idx in range(batch_size):
                    ...
                    all_preds.append(preds_dict)
            final_pickle_path = '../VisionFM/Final_prediction/RETFound_pred_{task}_{seed}.pickle'.format(task=task, seed=seed)
